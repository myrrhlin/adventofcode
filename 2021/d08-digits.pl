...
# in-place re-ordering of arrayref of patterns:
...
sub solve_segments ($patterns) {
  ...
  foreach my $six (0, 9) {
    my $nonfour = eval "'$patterns->[$six]' =~ tr/$patterns->[4]//dr";
    next unless length $nonfour == 2;
    $segmap[4] = eval "'abcdefg' =~ tr/$patterns->[$six]//dr";  # e
    unless ($six == 9) {  # identified the numeral 9
      ($patterns->[9], $patterns->[$six]) = ($patterns->[$six], $patterns->[9]);
    }
    last;
  }
  $segmap[3] = eval "'abcdefg' =~ tr/$patterns->[0]//dr"; # d
  foreach my $five (2, 3, 5) {
    my $nonnine = eval "'$patterns->[$five]' =~ tr/$patterns->[9]//dr";
    next unless length $nonnine;  # nine has _all_ segments of the 3 and 5
    unless ($five == 2) {  # identified the numeral 2
      ($patterns->[2], $patterns->[$five]) = ($patterns->[$five], $patterns->[2]);
    }
    last;
  }
  ...
  foreach my $five (3, 5) {
    my $nonfound = eval "'$patterns->[$five]' =~ tr/$found//dr";
    next unless length $nonfound == 1;
    $segmap[6] = $nonfound;  # segment g
    unless ($five == 3) {  # identified the numeral 3
      ($patterns->[3], $patterns->[$five]) = ($patterns->[$five], $patterns->[3]);
    }
    last;
  }
  ...
  $segmap[1] = eval "'abcdefg' =~ tr/$found//dr";  # g
  #print STDERR 'segments: '; p @segmap;
  #print STDERR 'solved patterns: ';
  #p $patterns;
}
sub solve_digits ($display) {
  # by now, all patterns are in proper place
